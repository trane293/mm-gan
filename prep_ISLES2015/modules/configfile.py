...
config['data_dir_prefix'] = os.path.join(mount_path_prefix, '/local-scratch/anmol/data/ISLES2015') # this should#  be top level path
config['hdf5_filepath_prefix'] = os.path.join(mount_path_prefix, '/local-scratch/anmol/data/ISLES2015/HDF5_Datasets/ISLES2015.h5') # top level path

config['spatial_size_for_training'] = (230, 230) # If any preprocessing is done, then this needs to change. This is the shape of data that you want to train with. If you are changing this that means you did some preprocessing.
config['num_slices'] = 154 # number of slices in input data. THIS SHOULD CHANGE AS WELL WHEN PERFORMING PREPROCESSING
...
config['train_patients'] = 28

# SET LATER
config['cropping_coords'] = [8, 204, 39, 189, 0, 158] # coordinates used to crop the volumes, this is generated using the notebook checkLargestCropSize.ipynb
config['size_after_cropping'] = (196, 150, 154) # set this if you set the above variable. Calculate this using the notebook again.
...
if config['data_order'] == 'th':
    ...
    config['train_shape'] = (
    config['train_patients'], 4, config['spatial_size_for_training'][0], config['spatial_size_for_training'][1],
    config['num_slices'])

    config['train_segmasks_shape'] = (
    config['train_patients'], config['spatial_size_for_training'][0], config['spatial_size_for_training'][1],
    config['num_slices'])


    config['train_shape_crop'] = (
    config['train_patients'], 4, config['size_after_cropping'][0], config['size_after_cropping'][1],
    config['size_after_cropping'][2])

    config['train_segmasks_shape_crop'] = (
    config['train_patients'], config['size_after_cropping'][0], config['size_after_cropping'][1],
    config['size_after_cropping'][2])
