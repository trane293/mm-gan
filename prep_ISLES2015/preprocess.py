...
filename = os.path.join(os.sep.join(config['hdf5_filepath_prefix'].split(os.sep)[0:-1]), 'ISLES_Cropped_Normalized.h5')
...
new_group_preprocessed.create_dataset("training_data", config['train_shape_crop'], np.float32)
new_group_preprocessed.create_dataset("training_data_pat_name", (config['train_shape_crop'][0],), dtype="S100")
new_group_preprocessed.create_dataset("training_data_segmasks", config['train_segmasks_shape_crop'], np.int16)

# ====================================================================================

# just copy the patient  names directly
new_group_preprocessed['training_data_pat_name'][:] = hdf5_file['training_data_pat_name'][:]
...
run_on_list = ['training_data_segmasks', 'training_data']

#only run the mean/var normalization on these datasets
std_list = ['training_data']
for run_on in run_on_list:
    ...
    if run_on == 'training_data':
        im_np = np.empty(config['train_shape_crop'])
