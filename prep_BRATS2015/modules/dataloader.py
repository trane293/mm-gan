        ...
        if os.path.isdir(patient):
            ...
            for curr_seq in sequence_folders: # get the filepath of the image (nii.gz)
                ...
                if '.OT.' in imagefile:
                    ...
                    if 'isles' not in dataset.lower():
                        img_obj_res = resize_mha_volume(img_obj, spacing=[1, 1, 1], size=[240, 240, 155])

                    img_obj_res = preprocessData(img_obj_res, process=preprocess)

                    pix_data = sitk.GetArrayViewFromImage(img_obj_res)
